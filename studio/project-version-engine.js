...
class ProfitMenteProjectVersionEngine{
  ...
  create(project,label='Punto de control'){const all=this._read(),key=this.projectKey(project),rows=all[key]||[],now=new Date().toISOString();const row={id:(globalThis.crypto?.randomUUID?.()||`${Date.now()}-${Math.random()}`),label:String(label||'Punto de control').trim().slice(0,80)||'Punto de control',createdAt:now,project:structuredClone(project)};rows.unshift(row);all[key]=rows.slice(0,this.limit);this._write(all);return structuredClone(row)}
