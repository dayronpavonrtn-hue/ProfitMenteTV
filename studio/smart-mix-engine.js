class ProfitMenteSmartMixEngine{
  ...
  static audioSoloSet(project){return new Set(this.AUDIO_TRACKS.filter(track=>!!this.state(project,track).solo))}
  static trackActive(project,track){
    const canonical=this.canonicalTrack(track);if(canonical===null||!this.AUDIO_TRACKS.includes(canonical))return false;
    if(this.state(project,canonical).muted)return false;
    const solos=this.audioSoloSet(project);return !solos.size||solos.has(canonical);
  }
  static muted(project,track){return !this.trackActive(project,track)}
  static active(clip,project){
    const track=this.canonicalTrack(clip?.track);
    return !!clip&&!clip.muted&&this.hasAsset(clip.asset)&&track!==null&&this.trackActive(project,track);
  }
  ...
  static inspect(project){
    const music=(project?.clips||[]).filter(c=>this.canonicalTrack(c?.track)===5&&this.active(c,project)),voice=(project?.clips||[]).filter(c=>this.canonicalTrack(c?.track)===6&&this.active(c,project));
  ...
  static apply(project,{duckRatio=.4}={}){
    ...
    for(const clip of project?.clips||[]){
      if(this.canonicalTrack(clip?.track)!==5||!this.active(clip,project)||this.voiceOverlap(project,clip)<=.01)continue;
      const next=this.safeDuckVolume(clip,duckRatio),rawOld=Number(clip.duckVolume??.16),old=Number.isFinite(rawOld)?rawOld:.16,was=clip.ducking!==false;
