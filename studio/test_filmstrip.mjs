...
for(const token of ['ProfitMenteFilmstripEngine','capture(asset,count)','video.onseeked=snap','toDataURL','decorate(project,assets','MutationObserver','observer.observe(tracks,{childList:true})','this.sameId(c.id,el.dataset.id)','this.sameId(a.id,clip.asset)']) if(!js.includes(token)) throw new Error('Filmstrip incompleto: '+token);
