(function integrateMediaRelink(){
  ...
  async function hashFile(file){
    try{
      if(ProfitMenteMediaImportEngine?.contentHashes)return await ProfitMenteMediaImportEngine.contentHashes(file);
      const current=await ProfitMenteMediaImportEngine?.contentHash?.(file)||'';
      return {current,legacy:'',version:current?'sample-v2':''};
    }catch{return {current:'',legacy:'',version:''}}
  }
  async function relinkFiles(files){
    let relinked=0,unmatched=0,ambiguous=0,failed=0;const used=new Set(),details=[];
    for(const file of Array.from(files||[])){
      try{
        const hashes=await hashFile(file);let candidates=assets.filter(a=>!used.has(a.id));
        const match=engine.bestMatch(candidates,file,hashes);
        ...
        const result=engine.apply(match.asset,file,hashes);if(!result.ok){failed++;details.push({file:file.name,status:'rejected',reason:result.reason});continue}
        await putAsset(match.asset);used.add(match.asset.id);relinked++;details.push({file:file.name,status:'relinked',assetId:match.asset.id,score:match.score,reason:result.identity?.reason});
      }catch(err){failed++;console.error('Error revinculando medio',file?.name,err)}
    }
    drawLibrary?.();drawTimeline?.();window.ProfitMentePreviewEngine?.clearCache?.();updatePreview?.();renderAt?.(+document.querySelector('#playhead')?.value||0);
