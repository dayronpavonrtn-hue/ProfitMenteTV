class ProfitMenteMediaLibraryTools{
  ...
  static unusedBytes(project,assets=[]){return this.unused(project,assets).reduce((sum,a)=>sum+Math.max(0,Number(a?.size??a?.blob?.size??0)||0),0)}
...
(function integrateMediaLibraryTools(){
  ...
  if(!document.querySelector('#profitmenteMediaLibraryToolsStyle')){const style=document.createElement('style');style.id='profitmenteMediaLibraryToolsStyle';style.textContent='.mediaLibraryTools{display:grid;grid-template-columns:minmax(0,1fr) 82px auto auto;gap:5px;align-items:center;margin:8px 0 4px}.mediaLibraryTools input,.mediaLibraryTools select{min-width:0;padding:7px 8px;font-size:10px}.mediaLibraryTools span{font-size:9px;color:#8e96a5}.mediaLibraryTools button{padding:7px 8px!important;margin:0!important;width:auto!important;white-space:nowrap;font-size:10px}.mediaLibraryTools button:disabled{opacity:.45}.mediaRow{display:grid;grid-template-columns:1fr 30px;gap:5px;align-items:stretch}.mediaRow>.mediaCard{width:100%;margin:0}.mediaRow>.mediaDelete{width:30px!important;margin:0!important;padding:3px!important;text-align:center!important;font-size:17px;background:#24171b;border-color:#51303a}.mediaFilterEmpty{padding:8px;color:#7f8795;text-align:center}';document.head.appendChild(style)}
  const controls=document.createElement('div');controls.className='mediaLibraryTools';controls.innerHTML='<input id="mediaSearch" type="search" placeholder="Buscar medios…" aria-label="Buscar medios"><select id="mediaTypeFilter" aria-label="Filtrar medios"><option value="all">Todos</option><option value="video">Video</option><option value="image">Imagen</option><option value="audio">Audio</option></select><span id="mediaCount"></span><button id="cleanupUnusedMedia" type="button" title="Elimina de IndexedDB los medios que no usa ningún clip">Limpiar no usados</button>';
  library.insertAdjacentElement('beforebegin',controls);
  const search=controls.querySelector('#mediaSearch'),filter=controls.querySelector('#mediaTypeFilter'),count=controls.querySelector('#mediaCount'),cleanup=controls.querySelector('#cleanupUnusedMedia');
  ...
  function cleanupStats(){const unused=tools.unused(project,assets),bytes=tools.unusedBytes(project,assets);cleanup.disabled=!unused.length;cleanup.textContent=unused.length?`Limpiar ${unused.length} no usado${unused.length===1?'':'s'}`:'Sin sobrantes';cleanup.title=unused.length?`Liberar aproximadamente ${(bytes/1048576).toFixed(1)} MB de medios que no están en el timeline`:'Todos los medios están en uso'}
