...
let r=qa.inspect(base,assets);if(r.issues.some(x=>x.includes('Recorte supera')))throw new Error('Valid source window incorrectly blocked');
r=qa.inspect({...base,clips:[{...base.clips[0],sourceOffset:6}]},assets);if(!r.issues.some(x=>x.includes('Recorte supera')))throw new Error('Source overflow was not blocked');
r=qa.inspect({...base,clips:[{...base.clips[0],duration:3,sourceOffset:5,speed:2}]},assets);if(!r.issues.some(x=>x.includes('requiere 11.00s')))throw new Error('Speed-aware source overflow was not blocked');
