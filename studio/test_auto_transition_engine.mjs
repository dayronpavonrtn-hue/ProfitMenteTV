...
const project={fps:24,clips:[
  ...
  {id:'d',track:0,name:'CTA',sceneText:'d',start:13,duration:3},
...
assert.equal(project.clips[3].transition,undefined,'transition across a real gap must be skipped');
assert.equal(project.clips[4].transition,'slide','manual clip must never be touched');
assert.ok(r.preserved>=2);
...
assert.equal(forced.clips[3].transition,undefined,'force must not bridge timeline gaps');
