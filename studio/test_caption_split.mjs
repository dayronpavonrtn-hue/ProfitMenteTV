...
let p=base(),r=e.split(p,'cap',2,{idFactory:()=> 'cap-b'});assert.equal(r.ok,true);assert.equal(p.clips.length,3);assert.equal(r.split,2);assert.equal(r.left.name,'uno dos');assert.equal(r.right.name,'tres cuatro');assert.equal(r.left.duration,1);assert.equal(r.right.start,2);assert.equal(r.right.duration,1);assert.equal(r.left.wordTimings.at(-1).word,'dos');assert.equal(r.right.wordTimings[0].word,'tres');assert.equal(r.right.wordTimings[0].start,2);assert.equal(r.left.wordTimings[0].index,0);assert.equal(r.right.wordTimings[0].index,0);assert.equal([...r.left.wordTimings,...r.right.wordTimings].filter(w=>w.word==='tres').length,1);
