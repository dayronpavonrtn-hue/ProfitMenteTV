(()=>{
  ...
  function draw(){const c=clip();block.hidden=!eligible(c);if(!c||block.hidden)return;const e=engine.forClip(c),d=Math.max(.001,Number(c.duration)||.001);$('#ciFadeIn').max=d;$('#ciFadeOut').max=d;if(document.activeElement!==$('#ciFadeIn'))$('#ciFadeIn').value=e.fadeIn.toFixed(2);if(document.activeElement!==$('#ciFadeOut'))$('#ciFadeOut').value=e.fadeOut.toFixed(2);$('#ciFadeInfo').textContent=`Entrada ${e.fadeIn.toFixed(2)}s · salida ${e.fadeOut.toFixed(2)}s · duración ${d.toFixed(2)}s`}
  function save(fi,fo,label){const c=clip();if(!eligible(c))return;const e=engine.normalize(c.duration,fi,fo);c.fadeIn=+e.fadeIn.toFixed(3);c.fadeOut=+e.fadeOut.toFixed(3);persist?.();drawTimeline?.();renderAt?.(+$('#playhead').value||0);setStatus?.(label||'Envolvente de audio actualizada');draw()}
