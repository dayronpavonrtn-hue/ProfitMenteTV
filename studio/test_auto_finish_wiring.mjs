...
assert.match(integration,/function generatorTools\(\)/,'Auto Finish must resolve the shared generator manual tools instead of duplicating caption/B-roll logic');
assert.match(integration,/ProfitMenteGeneratorManualToolsIntegration\?\.tools/,'Auto Finish must reuse the already integrated local generator tools');
assert.match(integration,/step==='scene-captions'/,'Auto Finish must execute scene-aware caption completion');
assert.match(integration,/tools\.addMissingCaptions\(project\)/,'Auto Finish captions must use the shared caption generator');
assert.match(integration,/step==='scene-broll'/,'Auto Finish must execute scene-aware B-roll completion');
assert.match(integration,/tools\.addBroll\(project,assets,\{maxClips:wanted\}\)/,'Auto Finish B-roll must use the shared local media selector');
assert.match(integration,/B-roll protegido/,'Auto Finish must surface locked B-roll tracks without editing them');
assert.match(integration,/subtítulos protegidos/,'Auto Finish must surface locked caption tracks without editing them');
