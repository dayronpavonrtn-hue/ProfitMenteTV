(()=>{
  ...
  async function trimClip(clip,cache=new Map()){
    if(!clip||!AUDIO_TRACKS.has(Number(clip.track))||!clip.asset)return {ok:false,reason:'not-audio-clip'};
  ...
  function reasonText(r){
    ...
    if(r==='locked')return 'la pista está bloqueada';
    if(r==='asset-missing')return 'falta el archivo de audio original';
    return 'no se pudo analizar el clip';
  }
  async function runOne(){
    if(busy)return;const clip=selected();if(!clip||!AUDIO_TRACKS.has(Number(clip.track))){status('Selecciona un clip de voz, música o efectos');return}
  ...
  async function runVoices(){
    if(busy)return;const clips=(project?.clips||[]).filter(c=>Number(c.track)===4&&c.asset&&!locked(c));if(!clips.length){status('No hay clips de voz editables para limpiar');return}
