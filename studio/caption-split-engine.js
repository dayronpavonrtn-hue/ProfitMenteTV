...
class ProfitMenteCaptionSplitEngine{
  ...
  normalizeWords(words,start,end){
    ...
    for(const item of words){
      ...
      const next=this.clone(item);next.start=Math.max(start,ws);next.end=Math.min(end,we);if(next.end<=next.start)continue;next.duration=+(next.end-next.start).toFixed(6);out.push(next);
    }
    out.sort((a,b)=>a.start-b.start||a.end-b.end);return out.map((item,index)=>({...item,index}));
  }
  ...
  uniqueId(project,idFactory){
    ...
    for(let attempt=0;attempt<20;attempt++){
      const candidate=typeof idFactory==='function'?idFactory(attempt):(globalThis.crypto?.randomUUID?.()||`caption-split-${Date.now()}-${Math.random().toString(16).slice(2)}`);
      const key=this.idKey(candidate);if(key!==null&&!used.has(key))return candidate;
    }
    return null;
  }
  split(project,clipId,time,{idFactory,minFrames=1}={}){
    ...
    const index=project.clips.findIndex(c=>this.idKey(c.id)===key);if(index<0)return {ok:false,reason:'not-found'};
    const source=project.clips[index],b=this.bounds(source);if(b.track!=='3')return {ok:false,reason:'not-caption'};
    if(this.locked(project,source))return {ok:false,reason:'locked'};
    const rawTime=this.strictNumber(time);if(rawTime===null)return {ok:false,reason:'invalid-time'};
    const frames=this.strictNumber(minFrames);if(frames===null||!Number.isInteger(frames)||frames<1)return {ok:false,reason:'invalid-min-frames'};
    const fps=this.fps(project),split=this.frame(rawTime,fps),edge=frames/fps;
    if(split<=b.start+edge-1e-9||split>=b.end-edge+1e-9)return {ok:false,reason:'too-close-to-edge',split,fps,minEdge:edge};
    ...
    if(hasWords){
      left.wordTimings=this.normalizeWords(source.wordTimings,b.start,split);right.wordTimings=this.normalizeWords(source.wordTimings,split,b.end);
      const leftText=left.wordTimings.map(w=>w.word).join(' ').trim(),rightText=right.wordTimings.map(w=>w.word).join(' ').trim();
      if(!leftText||!rightText)return {ok:false,reason:'empty-side'};
      left.name=leftText;right.name=rightText;
    }else{
      const parts=this.splitPlainText(source.name,ratio);if(!parts)return {ok:false,reason:'text-too-short'};left.name=parts[0];right.name=parts[1];delete left.wordTimings;delete right.wordTimings;
    }
    if(source.groupId!=null){delete left.groupId;delete right.groupId}
    project.clips=[...project.clips.slice(0,index),left,right,...project.clips.slice(index+1)];
