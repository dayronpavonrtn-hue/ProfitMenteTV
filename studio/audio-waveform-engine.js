class ProfitMenteAudioWaveformEngine{
  ...
  static clamp(value,min=0,max=1){const n=this.finiteNumber(value,min);return Math.min(max,Math.max(min,n))}
  static canonicalTrack(value){
    const n=this.finiteNumber(value,null);return n!==null&&Number.isInteger(n)&&n>=0&&n<=6?(Object.is(n,-0)?0:n):null;
  }
  static canonicalMediaId(value){
    if(value===null||value===undefined||typeof value==='boolean')return null;
    if(typeof value==='string'){
      const raw=value.trim();if(raw==='')return null;
      const numeric=this.finiteNumber(raw,null);
      return numeric!==null?String(Object.is(numeric,-0)?0:numeric):raw;
    }
  ...
  static channels(input=[]){
    ...
    if(typeof input.numberOfChannels==='number'&&Number.isInteger(input.numberOfChannels)&&input.numberOfChannels>=0&&typeof input.getChannelData==='function'){
  ...
  static buildPeaks(input=[],bins=512){
    const channels=this.channels(input),length=channels.reduce((n,ch)=>Math.max(n,Number.isInteger(ch?.length)&&ch.length>=0?ch.length:0),0),requested=this.finiteNumber(bins,512),count=Math.max(8,Math.min(4096,Math.floor(requested)));
    ...
    for(let bin=0;bin<count;bin++){
      ...
      for(const ch of channels){
        if(!ch)continue;
        for(let i=from;i<to&&i<ch.length;i++){const sample=Math.abs(this.finiteNumber(ch[i],0));if(sample>peak)peak=sample}
      }
  ...
  static sourceWindow({sourceOffset=0,clipDuration=0,speed=1,sourceDuration=0}={}){
    const total=Math.max(0,this.finiteNumber(sourceDuration,0)),rate=Math.max(.01,this.finiteNumber(speed,1)),offset=this.clamp(this.finiteNumber(sourceOffset,0),0,total||Number.MAX_SAFE_INTEGER),duration=Math.max(0,this.finiteNumber(clipDuration,0)),end=total?Math.min(total,offset+duration*rate):offset+duration*rate;
  ...
  static slicePeaks(peaks,{sourceOffset=0,clipDuration=0,speed=1,sourceDuration=0,bins=160}={}){
    const source=peaks||[],requested=this.finiteNumber(bins,160),outCount=Math.max(8,Math.min(1024,Math.floor(requested))),out=new Float32Array(outCount),total=Math.max(0,this.finiteNumber(sourceDuration,0));
    ...
    for(let bin=0;bin<outCount;bin++){
      ...
      for(let i=a;i<b&&i<source.length;i++)peak=Math.max(peak,this.finiteNumber(source[i],0));
