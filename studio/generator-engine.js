class ProfitMenteGeneratorEngine{
  ...
  generate(topic,duration=45){
    ...
    const clips=[];
    parts.forEach((p,i)=>{
      const start=i*slice,d=Math.min(slice,duration-start),transition=i===0?'cut':this.pick(['fade','zoom','slide'],seed,i);
      ...
      clips.push({id:crypto.randomUUID(),track:3,name:p[1],start:start+.15,duration:Math.max(.5,d-.3),asset:null,style:i===0?'hook-pop':'dynamic',animation:i===0?'pop':'word-pulse'});
    });
  ...
  assignAssets(project,assets){
    const visual=assets.filter(a=>a.type==='video'||a.type==='image');if(!visual.length)return {primary:0,broll:0};
    let primary=0,broll=0,used=new Set();
    for(const c of project.clips.filter(c=>c.track===0&&!c.asset)){
      const ranked=[...visual].sort((a,b)=>this.scoreAsset(b,c.keywords)-this.scoreAsset(a,c.keywords)||Number(used.has(a.id))-Number(used.has(b.id)));
      const chosen=ranked[0];if(!chosen)continue;c.asset=chosen.id;used.add(chosen.id);primary++;
      const alt=ranked.find(a=>a.id!==chosen.id);
      if(alt&&c.duration>=4){project.clips.push({id:crypto.randomUUID(),track:1,name:`B-roll · ${c.name}`,start:c.start+c.duration*.46,duration:Math.min(2.4,c.duration*.38),asset:alt.id,transition:'fade',motion:'slow-zoom',keywords:c.keywords});broll++}
    }
    return {primary,broll};
  }
}
