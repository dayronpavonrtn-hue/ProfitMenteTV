(()=>{
  ...
  function strictNumber(v){if(typeof v==='number')return Number.isFinite(v)?v:null;if(typeof v!=='string')return null;const s=v.trim();if(!s||!NUMERIC.test(s))return null;const n=Number(s);return Number.isFinite(n)?n:null}
  const clamp=(v,a,b)=>{const n=strictNumber(v);return Math.max(a,Math.min(b,n===null?0:n))};
  const presets={
  ...
  function identityKey(id){
    ...
    const s=id.trim();if(!s)return null;
    const n=Number(s);
    if(Number.isFinite(n)&&NUMERIC.test(s))return `n:${Object.is(n,-0)?0:n}`;
    return `s:${s}`;
  }
  ...
  const api={normalize,cssFilter,ffmpegFilter,applyPreset,presets,strictNumber,identityKey,sameId,findClip,canonicalTrack,stateLocked,isLocked,isVisual};
  if(typeof module!=='undefined')module.exports={ProfitMenteColorGrade:api};
  if(typeof window==='undefined'||typeof document==='undefined')return;
  window.ProfitMenteColorGrade=api;
  if(!document.querySelector('script[data-profitmente-color-batch-engine]')){
    const core=document.createElement('script');core.src='color-grade-batch-engine.js';core.dataset.profitmenteColorBatchEngine='1';
    core.onload=()=>{if(document.querySelector('script[data-profitmente-color-batch-integration]'))return;const s=document.createElement('script');s.src='color-grade-batch-integration.js';s.dataset.profitmenteColorBatchIntegration='1';document.body.appendChild(s)};
