(()=>{
  ...
  const api={normalize,cssFilter,ffmpegFilter,applyPreset,presets};
  ...
  const clip=()=>window.project?.clips?.find(c=>c.id===selected());
  function isVisual(c){return !!c&&[0,1].includes(Number(c.track))}
  function redraw(message){window.persist?.();window.drawTimeline?.();window.renderAt?.(+document.querySelector('#playhead').value||0);window.setStatus?.(message||'Color actualizado');render()}
  ...
  document.addEventListener('click',e=>{if(e.target.closest?.('.clip'))requestAnimationFrame(render)},true);setInterval(render,600);render();window.renderAt?.(+document.querySelector('#playhead')?.value||0);
})();