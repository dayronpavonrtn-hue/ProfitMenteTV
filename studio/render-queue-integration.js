(async()=>{
  ...
  function renderPanel(){
    ...
    const retryAll=button('↻ Reintentar errores','Devuelve a pendientes todos los renders fallidos',()=>{const n=changed(()=>queue.retryFailed());setStatus?.(n?`${n} render(es) devuelto(s) a pendientes`:'No hay errores para reintentar')});retryAll.disabled=queue.running||!queue.items.some(item=>item.status==='error');
    const clear=button('Limpiar terminados','Elimina de la lista trabajos listos, fallidos y cancelados',()=>{const n=changed(()=>queue.clearFinished());setStatus?.(n?`${n} trabajo(s) retirado(s) de la cola`:'No hay trabajos terminados para limpiar')});clear.disabled=queue.running||!queue.items.some(item=>['done','error','cancelled'].includes(item.status));toolbar.append(retryAll,clear);panelBody.appendChild(toolbar);
    ...
    for(const item of queue.items){
      ...
      if(item.status==='pending'){const up=button('▲','Subir prioridad',()=>changed(()=>queue.movePending(item.id,-1)));up.disabled=queue.running||pendingPos<=0;const down=button('▼','Bajar prioridad',()=>changed(()=>queue.movePending(item.id,1)));down.disabled=queue.running||pendingPos<0||pendingPos>=pending.length-1;order.append(up,down)}
      const actions=document.createElement('div');actions.style.cssText='display:flex;gap:4px';if(['error','cancelled'].includes(item.status)){const retry=button('↻','Reintentar',()=>changed(()=>queue.retry(item.id)));retry.disabled=queue.running;actions.appendChild(retry)}
      const remove=button('✕','Quitar de la cola',()=>{if(changed(()=>queue.remove(item.id)))setStatus?.(`Quitado de cola MP4: ${item.name}`)});remove.disabled=item.status==='running'||queue.running;actions.appendChild(remove);row.append(meta,order,actions);panelBody.appendChild(row);
    }
  }
  ...
  function enqueueCurrent(){if(queue.running)throw new Error('Espera a que termine la cola antes de añadir otra versión');const {renderProject,renderAssets}=capture();const item=queue.enqueue(renderProject,renderAssets,{name:renderProject?.name||project?.name||'profitmente'});void persistQueue();label();setStatus?.(`Añadido a cola MP4: ${item.name} · snapshot independiente`);return item}
