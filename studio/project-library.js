...
if(typeof document!=='undefined')(()=>{
  ...
  function saveCurrent(){if(typeof save==='function')save();project=lib.save(project);if(typeof originalPersist==='function')originalPersist();else if(typeof persist==='function')persist();render();status('Proyecto guardado en Mis proyectos')}
  function openProject(id){const next=lib.load(id);if(!next)return;project=next;if(typeof originalPersist==='function')originalPersist();else if(typeof persist==='function')persist();if(typeof drawTimeline==='function')drawTimeline();if(typeof drawLibrary==='function')drawLibrary();if(typeof syncForm==='function')syncForm();$('#playhead').value=0;if(typeof renderAt==='function')renderAt(0);window.historyEngine?.seed?.(project);status(`Proyecto abierto: ${project.name||'Sin título'}`)}
