...
assert.equal(Engine.canonicalMediaId(' Clip-A '),'Clip-A');
...
assert.equal(Engine.sameIdentity(7,' 007 '),true);
assert.equal(Engine.sameIdentity(7,'+07.000'),true);
assert.equal(Engine.sameIdentity(-0,'0'),true);
assert.equal(Engine.sameIdentity('Clip-A','clip-a'),false);
assert.equal(Engine.sameIdentity('',0),false);
assert.equal(Engine.sameIdentity(false,0),false);
const items=[{id:0,name:'zero'},{id:' 007 ',name:'seven'},{id:'Clip-A',name:'text'}];
assert.equal(Engine.findById(items,'-0')?.name,'zero');
assert.equal(Engine.findById(items,7)?.name,'seven');
assert.equal(Engine.findById(items,'+07.000')?.name,'seven');
assert.equal(Engine.findById(items,'Clip-A')?.name,'text');
assert.equal(Engine.findById(items,'clip-a'),null);
assert.equal(Engine.findById(items,false),null);
