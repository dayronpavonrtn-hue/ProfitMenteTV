...
class ProfitMenteSubtitleExportEngine{
  ...
  static time(seconds,srt=false){const totalMs=Math.max(0,Math.round((Number(seconds)||0)*1000)),ms=totalMs%1000,whole=Math.floor(totalMs/1000),s=whole%60,m=Math.floor(whole/60)%60,h=Math.floor(whole/3600);return `${this.pad(h)}:${this.pad(m)}:${this.pad(s)}${srt?',':'.'}${this.pad(ms,3)}`}
  ...
  static srt(project){const cues=this.cues(project);return cues.map((c,i)=>`${i+1}\n${this.time(c.start,true)} --> ${this.time(c.end,true)}\n${c.text}`).join('\n\n')+(cues.length?'\n':'')}
  static vtt(project){const cues=this.cues(project),body=cues.map(c=>`${this.time(c.start,false)} --> ${this.time(c.end,false)}\n${c.text}`).join('\n\n');return `WEBVTT\n\n${body}${body?'\n':''}`}
}
