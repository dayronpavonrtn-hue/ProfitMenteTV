(()=>{
  ...
  const selected=()=>Engine.findClip(project?.clips,window.ProfitMenteEditTools?.selectedId);
  ...
  async function runOne(){if(busy)return;const clip=selected();if(!clip||!Engine.AUDIO_TRACKS.includes(Engine.canonicalTrack(clip.track))){status('Selecciona un clip de voz, música o efectos');return}if(Engine.clipLocked(project,clip)){status('No se normalizó: el clip o su pista está bloqueado');return}busy=true;updateButtons();try{const r=await normalizeClip(clip);if(!r.ok){status(r.reason==='silence'?'No se normalizó: el tramo seleccionado está en silencio':r.reason==='locked'?'No se normalizó: el clip o su pista está bloqueado':'No se pudo analizar el audio seleccionado');return}refresh();status(`Audio normalizado · ${r.target.label} · ${r.metrics.rmsDb.toFixed(1)} dB RMS → objetivo ${r.target.rmsDb} dB${r.limitedByPeak?' · limitado por pico':''}`)}catch(e){console.error(e);status('No se pudo normalizar el audio: '+(e?.message||e))}finally{busy=false;updateButtons()}}
  async function runAll(options={}){
    ...
    const active=Engine.activeAudioClips(project),clips=Engine.mutableAudioClips(project);
    const inactive=Math.max(0,audioClips.length-active.length),locked=Math.max(0,active.length-clips.length);
    if(!clips.length){
      const reason=locked?'locked':audioClips.length?'inactive':'empty';
      if(!quiet)status(locked?'No hay clips de audio desbloqueados para normalizar':audioClips.length?'No hay clips de audio activos para normalizar':'No hay clips de audio para normalizar');
      return {ok:false,reason,changed:0,skipped:inactive+locked,locked};
    }
    busy=true;updateButtons();const cache=new Map();let changed=0,skipped=inactive+locked;
    try{
      if(!quiet)status(`Analizando ${clips.length} clip(s) de audio activo(s) y desbloqueado(s) localmente…`);
      ...
      return {ok:true,changed,skipped,locked,deferred:!!deferPersist};
    }finally{busy=false;updateButtons()}
  }
  function updateButtons(){const c=selected(),ok=c&&Engine.AUDIO_TRACKS.includes(Engine.canonicalTrack(c.track))&&!Engine.clipLocked(project,c);if(oneBtn){oneBtn.disabled=busy||!ok;oneBtn.textContent=busy?'Analizando…':'⚖ Normalizar clip'}if(allBtn){allBtn.disabled=busy;allBtn.textContent=busy?'Analizando mezcla…':'⚖ Normalizar mezcla'}}
  ...
  function install(){
    ...
    const qa=$('#qaBtn'),aside=qa?.parentElement;if(aside&&!$('#audioNormalizeAllBtn')){allBtn=document.createElement('button');allBtn.id='audioNormalizeAllBtn';allBtn.type='button';allBtn.textContent='⚖ Normalizar mezcla';allBtn.title='Normaliza solo las pistas de audio activas y desbloqueadas según Solo/Mute y objetivos seguros';qa.insertAdjacentElement('afterend',allBtn)}else allBtn=$('#audioNormalizeAllBtn');
