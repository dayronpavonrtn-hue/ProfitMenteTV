...
// Duplicate places a selection immediately after its span, remaps groups and preserves the prior clipboard.
const duplicateEngine=new ProfitMenteClipboardEngine();duplicateEngine.copy([{id:'keep',track:0,start:0,duration:1,name:'Keep'}]);
const duplicateProject={duration:14,clips:structuredClone(grouped),trackState:{}};r=duplicateEngine.duplicate(duplicateProject,duplicateProject.clips);assert.equal(r.ok,true);assert.equal(r.duplicate,true);assert.equal(r.base,4);assert.deepEqual(r.clips.map(c=>c.start),[4,5]);assert.equal(r.remappedGroups,1);assert.notEqual(r.clips[0].groupId,'source-group');assert.equal(duplicateEngine.count,1);const savedPaste=duplicateEngine.paste({duration:10,clips:[],trackState:{}},0);assert.equal(savedPaste.clips[0].name,'Keep copia');
