...
for(const script of ['preview-engine.js','project-library.js','relink-engine.js','bundle-engine.js','qa-engine.js','media-library-tools.js']) if(!scripts.includes(script)) throw new Error(`Módulo crítico no conectado: ${script}`);
const mediaToolsIndex=scripts.indexOf('media-library-tools.js'),mediaInspectorIndex=scripts.indexOf('media-inspector.js');
if(mediaToolsIndex<0||mediaInspectorIndex<0||mediaToolsIndex>mediaInspectorIndex) throw new Error('Herramientas de biblioteca deben cargarse antes del inspector de medios');
...
for(const token of ['function bootSupportModules()','bootSupportModules();','project-portability.js','bootRecovery','recovery-engine.js','recovery-integration.js']) if(!generator.includes(token)) throw new Error('Carga de soporte/recuperación no tiene integración determinista: '+token);
...
for(const token of ['canonicalTrack(c.track)','!trackHidden(track)','if(trackHidden(3))return','isTrackHidden:trackHidden']) if(!preview.includes(token)) throw new Error('Preview no respeta identidad canónica/pistas ocultas directamente: '+token);
if(preview.includes('trackHidden(Number(c.track))')) throw new Error('Preview volvió a coerción numérica insegura de identidad de pista');
