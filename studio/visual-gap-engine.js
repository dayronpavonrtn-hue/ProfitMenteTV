class ProfitMenteVisualGapEngine{
  ...
  static fill(project,assets=[],options={}){
    const gaps=this.gaps(project,Number(options.minGap)||.15),tracks=this.visibleTracks(project),usage=this.usage(project),created=[],unresolved=[];if(!tracks.length)return {created,unresolved:[...gaps.map(g=>({gap:g,reason:'visual-tracks-hidden'}))],gaps};
    const track=tracks[0];let lastId=null;
    for(const gap of gaps){let cursor=gap[0],guard=0;while(cursor<gap[1]-.01&&guard++<200){const asset=this.pick(assets,usage,lastId);if(!asset){unresolved.push({gap:[cursor,gap[1]],reason:'no-visual-assets'});break}
        ...
        const duration=Math.max(.01,Math.min(remaining,asset.type==='image'?remaining:native));const clip={id:(globalThis.crypto?.randomUUID?.()||`gap-${Date.now()}-${created.length}`),track,asset:asset.id,name:`Auto fill · ${asset.name||'medio'}`,start:+cursor.toFixed(3),duration:+duration.toFixed(3),sourceOffset:0,speed:1,fitMode:'cover',transition:'fade',transitionDuration:Math.min(.25,duration/3),autoGapFill:true};
        project.clips.push(clip);created.push(clip);usage.set(asset.id,(usage.get(asset.id)||0)+1);lastId=asset.id;cursor+=duration;
      }}
