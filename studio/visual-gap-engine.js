class ProfitMenteVisualGapEngine{
  static state(project,track){
    const key=String(track),read=map=>map?.[track]??map?.[key];
    const current=read(project?.trackState),legacy=read(project?.trackStates);
    const a=current&&typeof current==='object'?current:{},b=legacy&&typeof legacy==='object'?legacy:{};
    return {...b,...a,hidden:a.hidden===true||b.hidden===true,locked:a.locked===true||b.locked===true};
  }
  static visibleTracks(project){return [0,1].filter(t=>!this.state(project,t).hidden)}
  static writableTracks(project){return this.visibleTracks(project).filter(t=>!this.state(project,t).locked)}
  ...
  static fill(project,assets=[],options={}){
    const duration=Math.max(0,Number(project?.duration)||0),visible=this.visibleTracks(project),writable=this.writableTracks(project),created=[],unresolved=[];
    if(!visible.length)return {created,unresolved:duration?[{gap:[0,duration],reason:'visual-tracks-hidden'}]:[],gaps:[]};
    const gaps=this.gaps(project,Number(options.minGap)||.15);
    if(!writable.length)return {created,unresolved:gaps.map(gap=>({gap:[...gap],reason:'visual-tracks-locked'})),gaps};
    const usage=this.usage(project),track=writable[0];let lastId=null;
