...
p=pathlib.Path(sys.argv[1]); project=normalize_track_solo(json.loads(p.read_text(encoding='utf-8')),normalize_scalars=False); assets_dir=pathlib.Path(sys.argv[2]) if len(sys.argv)==3 else None
errors=[]; warnings=[]
