...
const oldProject={version:'1.3',name:'Trabajo importante',mode:'Automático',duration:61,format:'16:9',fps:60,clips:[{id:'c1',track:0,start:0,duration:10}]};
const calls=[];
...
assert.deepEqual(result.project,{version:'1.3',name:'Nuevo video',mode:'Manual',duration:45,format:'9:16',fps:30,clips:[]});
assert.deepEqual(oldProject.clips,[{id:'c1',track:0,start:0,duration:10}], 'reset must not mutate the current project');

const custom=engine.createBlank({name:'Prueba',mode:'Automático',duration:30,format:'1:1',fps:60});
...
assert.equal(custom.fps,60);
assert.deepEqual(custom.clips,[]);

assert.equal(engine.createBlank({fps:24}).fps,24);
assert.equal(engine.createBlank({fps:30}).fps,30);
assert.equal(engine.createBlank({fps:'60'}).fps,60);
assert.equal(engine.createBlank({fps:25}).fps,30);
assert.equal(engine.createBlank({fps:'invalid'}).fps,30);
...
assert.equal(withoutRecovery.project.fps,30);
