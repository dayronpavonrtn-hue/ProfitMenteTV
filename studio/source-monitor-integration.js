(()=>{
  ...
  const panel=document.createElement('section');panel.id='sourceMonitor';panel.hidden=true;panel.innerHTML=`<div class="sourceMonitorHead"><b>Monitor de fuente</b><button type="button" data-sm-close title="Cerrar">×</button></div><div class="sourceMonitorName"></div><div class="sourceMonitorMedia"></div><input class="sourceMonitorSeek" type="range" min="0" max="1" step="0.01" value="0"><div class="sourceMonitorReadout"><span data-sm-now>00:00.00</span><span data-sm-range>IN 00:00.00 · OUT 00:00.00</span></div><label class="sourceMonitorTarget">Pista destino <select data-sm-track></select></label><div class="sourceMonitorActions"><button type="button" data-sm-in>⟦ Marcar IN</button><button type="button" data-sm-out>Marcar OUT ⟧</button><button type="button" data-sm-reset>Restablecer</button><button type="button" data-sm-place>Colocar selección</button><button type="button" data-sm-replace>↻ Reemplazar clip</button></div><small class="sourceMonitorHint">Reemplazar usa IN/OUT como fuente y conserva la posición y duración del clip seleccionado.</small>`;
  library.insertAdjacentElement('afterend',panel);
  const style=document.createElement('style');style.id='profitmenteSourceMonitorStyle';style.textContent=`#sourceMonitor{margin-top:8px;padding:8px;border:1px solid #303846;border-radius:9px;background:#111720}.sourceMonitorHead{display:flex;justify-content:space-between;align-items:center}.sourceMonitorHead button{width:auto!important;margin:0!important;padding:2px 7px!important}.sourceMonitorName{font-size:10px;color:#b7c0ce;margin:4px 0 6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.sourceMonitorMedia video,.sourceMonitorMedia img{display:block;width:100%;max-height:190px;object-fit:contain;background:#05070a;border-radius:6px}.sourceMonitorMedia audio{width:100%}.sourceMonitorSeek{width:100%;margin:7px 0 2px}.sourceMonitorReadout{display:flex;justify-content:space-between;gap:6px;font-size:9px;color:#9aa5b5}.sourceMonitorTarget{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:6px;font-size:10px;color:#b7c0ce}.sourceMonitorTarget select{width:auto;min-width:92px;margin:0!important;padding:5px!important}.sourceMonitorActions{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:6px}.sourceMonitorActions button{margin:0!important;padding:7px 5px!important;font-size:10px}.sourceMonitorActions [data-sm-replace]{grid-column:1/-1;background:#142029;border-color:#305166}.sourceMonitorHint{display:block;margin-top:6px;font-size:9px;line-height:1.3;color:#8fa0b4}.sourceMonitorOpen{width:30px!important;margin:0!important;padding:3px!important;font-size:14px}.mediaRow:has(.sourceMonitorOpen){grid-template-columns:1fr 30px 30px}`;document.head.appendChild(style);
  const mediaHost=panel.querySelector('.sourceMonitorMedia'),seek=panel.querySelector('.sourceMonitorSeek'),nameEl=panel.querySelector('.sourceMonitorName'),nowEl=panel.querySelector('[data-sm-now]'),rangeEl=panel.querySelector('[data-sm-range]'),trackEl=panel.querySelector('[data-sm-track]');
  ...
  panel.querySelector('[data-sm-replace]').onclick=async()=>{
    if(!asset)return;const replacement=window.profitMenteMediaReplace;if(!replacement?.replaceFromRange){status('El motor de reemplazo todavía no está disponible');return}
    const r=range();if(!r.valid){status('No hay una selección IN/OUT válida para reemplazar el clip');return}
    await replacement.replaceFromRange(asset,r.in,r.out);
  };
