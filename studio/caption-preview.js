(()=>{
  ...
  renderAt=async function(t){
    ...
    ctx.save();
    ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`900 ${Math.round(46*pop)}px Arial`;
    const text=String(word.word).toUpperCase(),metrics=ctx.measureText(text),pad=24,x=canvas.width/2,y=canvas.height*.73,w=metrics.width+pad*2,h=72*pop;
