...
for(const fps of [24,30,60]){
  ...
  let r=E.apply(p,'a',1);assert.equal(r.ok,true);close(p.clips[0].start,1+1/fps);
  r=E.apply(p,'a',-1);assert.equal(r.ok,true);close(p.clips[0].start,1);
}
...
let r=E.apply(grouped,'v',10);assert.equal(r.changed,2);close(grouped.clips[0].start,2+10/30);close(grouped.clips[1].start,2.5+10/30);
...
r=E.apply(edge,'a',-300);assert.equal(r.ok,true);close(edge.clips[0].start,0);
