(()=>{
  ...
  const E=window.ProfitMenteRenderRangeEngine,$=s=>document.querySelector(s);let previewTimer=null,loopPreview=false,restarting=false;
  ...
  function refresh(){
    ...
    const loop=$('#loopRangeBtn');if(loop){loop.classList.toggle('active',loopPreview);loop.textContent=loopPreview?'🔁 Loop ON':'🔁 Loop OFF';loop.setAttribute('aria-pressed',String(loopPreview))}
  }
  function stopPlaybackOnly(){if(typeof playing!=='undefined'&&playing)$('#playBtn')?.click()}
  function stopRangePreview(reset=true){if(previewTimer){clearInterval(previewTimer);previewTimer=null}stopPlaybackOnly();if(reset)window.ProfitMenteTransport?.seek?.(currentRange().start)}
  async function restartLoop(r){
    if(restarting)return;restarting=true;
    try{
      stopPlaybackOnly();window.ProfitMenteTransport?.seek?.(r.start);await renderAt?.(r.start);$('#playBtn')?.click();
  ...
  async function previewRange(){
    ...
    stopRangePreview(false);window.ProfitMenteTransport?.seek?.(r.start);$('#playhead').value=r.start;await renderAt?.(r.start);$('#playBtn')?.click();
    previewTimer=setInterval(async()=>{
      const decision=E.previewDecision(project,r.start,r.end,now(),loopPreview);
      if(decision.action==='loop'){await restartLoop(r);return}
      if(decision.action==='stop'||decision.action==='seek-start'){
        stopRangePreview(true);status(`Preview de rango terminado · ${r.duration.toFixed(2)}s`)
      }
    },35);
    status(`${loopPreview?'Loop':'Preview'} del rango ${E.time(r.start)} → ${E.time(r.end)}`);
  }
  ...
  function mount(){
    ...
    $('#clearRangeBtn').onclick=()=>{stopRangePreview(false);persistRange(0,Number(project.duration)||0);status('Rango restablecido a todo el proyecto')};
    $('#previewRangeBtn').onclick=previewRange;$('#loopRangeBtn').onclick=()=>{loopPreview=!loopPreview;refresh();status(loopPreview?'Loop de rango activado':'Loop de rango desactivado')};$('#renderRangeBtn').onclick=renderRange;
    document.addEventListener('keydown',e=>{if(['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName)||e.ctrlKey||e.metaKey||e.altKey)return;if(e.key.toLowerCase()==='i'){e.preventDefault();$('#setRangeInBtn').click()}else if(e.key.toLowerCase()==='o'){e.preventDefault();$('#setRangeOutBtn').click()}else if(e.key.toLowerCase()==='l'&&e.shiftKey){e.preventDefault();$('#loopRangeBtn').click()}});
    if(!document.querySelector('#profitmenteWorkRangeStyle')){const s=document.createElement('style');s.id='profitmenteWorkRangeStyle';s.textContent='.workRangeControls{margin-top:10px;padding-top:8px;border-top:1px solid #2d3340}.workRangeControls h3{margin:4px 0}.workRangeControls #workRangeLabel{font-size:11px;color:#aeb7c8;margin:5px 0}.workRangeButtons{display:grid;grid-template-columns:1fr 1fr;gap:5px}.workRangeButtons button{font-size:10px;padding:6px}.workRangeButtons button.active{outline:1px solid currentColor;font-weight:700}';document.head.appendChild(s)}
    refresh();
  }
  const basePersist=window.persist;if(typeof basePersist==='function')window.persist=function(){basePersist();refresh()};
  window.addEventListener('profitmente:project-restored',()=>{stopRangePreview(false);refresh()});window.addEventListener('load',()=>{mount();refresh()},{once:true});if(document.readyState==='complete'){mount();refresh()}
  window.ProfitMenteRenderRange={currentRange,previewRange,renderRange,refresh,get loopPreview(){return loopPreview},setLoop(v){loopPreview=!!v;refresh()}};
})();
