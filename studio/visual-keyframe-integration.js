(()=>{
  const root=typeof window!=='undefined'?window:globalThis,Engine=root.ProfitMenteVisualKeyframeEngine,AdjustEngine=root.ProfitMenteVisualAdjustEngine;
  ...
  const engine=new Engine(),adjustEngine=AdjustEngine?new AdjustEngine():null,ctx=canvas.getContext('2d'),$=s=>document.querySelector(s);
  const section=document.createElement('section');section.className='visualKeyframePanel';section.innerHTML=`<hr><h3>Keyframes visuales</h3><div class="ciGrid"><label>X %<input id="vkX" type="number" min="-200" max="200" step="1" value="0"></label><label>Y %<input id="vkY" type="number" min="-200" max="200" step="1" value="0"></label><label>Escala<input id="vkScale" type="number" min="0.1" max="8" step="0.05" value="1"></label><label>Rotación °<input id="vkRotation" type="number" min="-3600" max="3600" step="1" value="0"></label><label>Opacidad<input id="vkOpacity" type="number" min="0" max="1" step="0.05" value="1"></label><label>Interpolación<select id="vkEasing"><option value="linear">Lineal</option><option value="ease-in">Acelerar</option><option value="ease-out">Desacelerar</option><option value="ease-in-out">Suave</option><option value="hold">Mantener</option></select></label></div><div class="ciActions"><button id="vkSet">◆ Añadir / actualizar</button><button id="vkRemove">Quitar keyframe</button><button id="vkClear">Restablecer</button></div><small id="vkInfo"></small>`;props.appendChild(section);
  const inputs={x:$('#vkX'),y:$('#vkY'),scale:$('#vkScale'),rotation:$('#vkRotation'),opacity:$('#vkOpacity')},easingInput=$('#vkEasing'),setBtn=$('#vkSet'),removeBtn=$('#vkRemove'),clearBtn=$('#vkClear'),info=$('#vkInfo');
  ...
  function inputState(){return {x:inputs.x.value,y:inputs.y.value,scale:inputs.scale.value,rotation:inputs.rotation.value,opacity:inputs.opacity.value,easing:easingInput?.value||'linear'}}
  ...
  function refresh(){
    ...
    for(const el of [...Object.values(inputs),easingInput,setBtn,removeBtn,clearBtn])if(el)el.disabled=!eligible||locked;
    if(!eligible){info.textContent='Selecciona un clip visual de Video u Overlays.';fill(engine.state());if(easingInput)easingInput.value='linear';return}
    const t=localTime(clip),frames=engine.normalize(clip),state=engine.stateAt(clip,t);fill(state);if(easingInput)easingInput.value=engine.easingAt?.(clip,t)||'linear';
  ...
  async function performRender(time){
    ...
    ctx.drawImage=function(source,...args){
      ...
      ctx.save();ctx.globalAlpha*=state.opacity;if(adjustEngine)ctx.filter=adjustEngine.canvasFilter(clip);ctx.translate(canvas.width/2+dx,canvas.height/2+dy);ctx.rotate(state.rotation*Math.PI/180);ctx.scale(state.scale,state.scale);ctx.translate(-canvas.width/2,-canvas.height/2);
  ...
  root.ProfitMenteVisualKeyframes={engine,adjustEngine,refresh,stateAt:(clip,time)=>engine.stateAt(clip,time),renderWithKeyframes,performRender,activeCandidates,destroy(){section.remove();if(root.renderAt===renderWithKeyframes)root.renderAt=originalRender;delete root.ProfitMenteVisualKeyframes}};
  refresh();
})();