(()=>{
  ...
  function fps(){
    const raw=Number(project?.fps),value=[24,30,60].includes(Math.round(raw))?Math.round(raw):30;
    return value;
  }
  ...
  applyZoom();
  window.ProfitMenteTransport={seek,prevCut,nextCut,get fps(){return fps()},get zoom(){return zoom},setZoom(v){zoom=Number(v);applyZoom()}};
