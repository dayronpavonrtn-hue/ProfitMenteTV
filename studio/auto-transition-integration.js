(()=>{
  ...
  function run(force=false){
    ...
    setStatus?.(`Transiciones auto · ${result.changed} ajustada(s)${result.preserved?` · ${result.preserved} manual(es) preservada(s)`:''}${result.skipped?` · ${result.skipped} hueco(s) omitido(s)`:''}`);
    return result;
  }
  function install(){
    if($('#autoTransitionBtn'))return;const anchor=$('#autoFinishBtn')||$('#generateBtn')||$('#qaBtn');if(!anchor)return;
    const btn=document.createElement('button');btn.id='autoTransitionBtn';btn.type='button';btn.textContent='✨ Transiciones auto';btn.title='Aplica transiciones locales a escenas generadas contiguas sin sobrescribir ajustes manuales.';btn.onclick=()=>run(false);anchor.insertAdjacentElement('afterend',btn);
  }
