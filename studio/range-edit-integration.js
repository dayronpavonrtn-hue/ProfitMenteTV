(()=>{
  ...
  function lift(){
    const r=currentRange();let result;try{result=engine.lift(project,r.start,r.end)}catch(e){status(e.message);return}
    rootSelectionClear();refreshAt(result.start,`Rango levantado · hueco ${result.duration.toFixed(2)}s · ${result.removed} eliminado(s), ${result.trimmed+result.split} recortado(s) · duración conservada`)
  }
  function insert(){
  ...
  function mount(){
    ...
    const box=document.createElement('div');box.id='rangeEditControls';box.innerHTML='<h3>Edición por rango</h3><div class="rangeEditButtons"><button id="liftRangeEditBtn" title="Eliminar IN→OUT y conservar el hueco/tiempo (Shift+Alt+Delete)">⬆ Levantar IN→OUT</button><button id="extractRangeEditBtn" title="Eliminar IN→OUT y cerrar el tiempo en todas las pistas desbloqueadas (Alt+Delete)">🗜 Extraer IN→OUT</button><label>Insertar <input id="insertTimeDuration" type="number" min="0.05" max="3600" step="0.05" value="1"> s</label><button id="insertTimeBtn" title="Insertar tiempo vacío en el cursor (Alt+Insert)">➕ Insertar tiempo</button></div><small>Levantar conserva la duración y deja un hueco. Extraer cierra ese tiempo. Las pistas o clips bloqueados protegen el material afectado.</small>';
    host.appendChild(box);$('#liftRangeEditBtn').onclick=lift;$('#extractRangeEditBtn').onclick=extract;$('#insertTimeBtn').onclick=insert;
  ...
  document.addEventListener('keydown',e=>{if(['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName)||e.ctrlKey||e.metaKey||!e.altKey)return;if(e.key==='Delete'){e.preventDefault();if(e.shiftKey)lift();else extract()}else if(e.key==='Insert'&&!e.shiftKey){e.preventDefault();insert()}},true);
  ...
  window.ProfitMenteRangeEdit={engine,extract,lift,insert,mount};
})();
