(()=>{
  ...
  async function render(){
    ...
    for(const el of clips){
      if(token!==renderToken)return;const clip=Engine.findById(project.clips||[],el.dataset.id);
      if(!clip||!Engine.isAudioTrack(clip.track)||!Engine.hasAsset(clip.asset)){clearElement(el);continue}
