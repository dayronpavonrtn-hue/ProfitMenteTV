(()=>{
  ...
  const selectedVisualClip=()=>{const id=selectedId();return (project?.clips||[]).find(c=>Engine.sameId(c?.id,id)&&[0,1].includes(Engine.canonicalTrack(c?.track)))||null};
  const refresh=()=>{const selected=!!selectedVisualClip();copy.disabled=!selected;paste.disabled=!selected||!clipboard};
