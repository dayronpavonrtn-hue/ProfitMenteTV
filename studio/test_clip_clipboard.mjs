...
const base=()=>({
  ...
  trackState:{0:{locked:false},1:{locked:false}}
});

{
  const project=base(),engine=new ProfitMenteClipClipboardEngine();
  const copied=engine.collect(project,'+01.0');
  assert.equal(copied.ok,true);assert.equal(copied.count,2);assert.equal(copied.origin,1);
...
{
  const project=base(),engine=new ProfitMenteClipClipboardEngine();
  assert.equal(engine.collect(project,1).ok,true);
...
{
  const project=base(),engine=new ProfitMenteClipClipboardEngine();
  assert.equal(engine.collect(project,'solo').ok,true);
