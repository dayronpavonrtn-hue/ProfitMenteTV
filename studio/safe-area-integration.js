(()=>{
  ...
  const platform=$('#safeAreaPlatform'),enabled=$('#safeAreaEnabled'),statusEl=$('#safeAreaStatus');
  function apply(){
    ...
    const qa=engine.inspect(project,platform.value);statusEl.textContent=qa.warnings.length?`${qa.warnings.length} texto(s) fuera de la zona segura.`:'Todos los títulos están dentro de la zona segura.';return qa
  }
  platform.onchange=()=>{project.safeAreaPlatform=platform.value;if(typeof persist==='function')persist();apply()};enabled.onchange=()=>{project.safeAreaGuides=enabled.checked;if(typeof persist==='function')persist();apply()};
  ...
  window.ProfitMenteSafeArea={engine,apply};apply();
})();