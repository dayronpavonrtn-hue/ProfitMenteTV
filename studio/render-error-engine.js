class ProfitMenteRenderErrorEngine{
  static diagnose(error){
    ...
    if(/abre studio con start_studio_windows\.bat|servidor local.*(?:no disponible|no activo|inactivo)|activar el render mp4 directo|local render server.*(?:unavailable|not running)/.test(text))return {...result,code:'local_server_missing',title:'El servidor local de render no está activo',action:'Cierra esta pestaña y abre ProfitMente Studio con start_studio_windows.bat. El proyecto permanece guardado y no necesitas reconstruirlo.',retryable:true};
