(()=>{
  ...
  async function inspect(){
    ...
    try{
      ...
      const parts=[`${summary.total} clip(s)`,`${summary.clipping} clipping`,`${summary.hot} cerca de 0 dB`,`${summary.silent} silencioso(s)`];if(summary.unavailable)parts.push(`${summary.unavailable} sin analizar`);if(mix.invalidTimingCount)parts.push(`${mix.invalidTimingCount} con tiempo de timeline inválido`);if(worst)parts.push(`pico clip ${worst.dbfs.toFixed(1)} dBFS`);if(mix.clipping)parts.push(`${mix.clipping} tramo(s) de mezcla con clipping estimado`);else if(mix.hot)parts.push(`${mix.hot} tramo(s) de mezcla cerca de 0 dB`);if(mix.worst)parts.push(`mezcla máx. ${mix.worst.dbfs.toFixed(1)} dBFS`);
      resultEl.textContent=parts.join(' · ');const totalRisk=summary.clipping+mix.clipping+mix.invalidTimingCount;if(typeof setStatus==='function')setStatus(totalRisk?`Audio QA: ${totalRisk} riesgo(s) entre clips, mezcla y timing`:'Audio QA completado: sin clipping detectado');
