...
const project={version:'1.3',name:'Portable',duration:12,format:'9:16',clips:[{id:'c1',track:0,asset:'a1',start:0,duration:6},{id:'c2',track:1,asset:'missing1',start:6,duration:4}],assets:[{id:'missing1',name:'missing.mov',type:'video',mime:'video/quicktime',size:7654321,duration:4.2,width:1920,height:1080,metadataVersion:4,metadataBlobSize:7654321,metadataBlobType:'video/quicktime',metadataBlobLastModified:1700000000000,metadataBlobSignature:'11223344aabbccdd',blob:{mustNotLeak:true}}]};
const assets=[{id:'a1',name:'clip.mp4',type:'video',mime:'video/mp4',size:1234567,duration:8.25,width:1080,height:1920,metadataVersion:4,metadataBlobSize:1234567,metadataBlobType:'video/mp4',metadataBlobLastModified:1700000000123,metadataBlobSignature:'deadbeefcafebabe',blob:{private:true},thumbnail:'data:image/jpeg;base64,NO'}];
...
assert.deepEqual(current,{id:'a1',name:'clip.mp4',type:'video',mime:'video/mp4',metadataBlobType:'video/mp4',size:1234567,duration:8.25,width:1080,height:1920,metadataVersion:4,metadataBlobSize:1234567,metadataBlobLastModified:1700000000123,metadataBlobSignature:'deadbeefcafebabe'});
assert.deepEqual(missing,{id:'missing1',name:'missing.mov',type:'video',mime:'video/quicktime',metadataBlobType:'video/quicktime',size:7654321,duration:4.2,width:1920,height:1080,metadataVersion:4,metadataBlobSize:7654321,metadataBlobLastModified:1700000000000,metadataBlobSignature:'11223344aabbccdd'});
...
assert.equal(restored.assets.find(a=>a.id==='a1').metadataBlobSignature,'deadbeefcafebabe');
assert.equal(restored.assets.find(a=>a.id==='a1').metadataBlobSize,1234567);
assert.equal(restored.assets.find(a=>a.id==='a1').metadataBlobType,'video/mp4');
assert.equal(restored.assets.find(a=>a.id==='a1').metadataBlobLastModified,1700000000123);
...
assert.equal(restored.assets.find(a=>a.id==='missing1').metadataBlobSignature,'11223344aabbccdd');
...
console.log('Project portability QA OK · media inspector identity roundtrip retained · missing-media metadata retained · persisted exports guarded');
