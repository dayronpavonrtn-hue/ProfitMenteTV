(function(root,factory){
  ...
  class ProfitMenteFrameNudgeEngine{
    ...
    static locked(project,clip){const state=project?.trackState?.[clip?.track]??project?.trackState?.[String(clip?.track)];return !!state?.locked}
    ...
    static delta(project,clipId,frames){
      const members=this.members(project,clipId);if(!members.length)return {ok:false,reason:'missing',delta:0,members:[]};
      if(members.some(c=>this.locked(project,c)))return {ok:false,reason:'locked',delta:0,members};
      const count=Math.trunc(Number(frames)||0);if(!count)return {ok:false,reason:'zero',delta:0,members};
      const requested=count*this.frame(project),b=this.bounds(project,members),duration=Math.max(0,Number(project?.duration)||0);
      const min=-b.start,max=Math.max(0,duration-b.end),delta=Math.max(min,Math.min(max,requested));
      if(Math.abs(delta)<1e-12)return {ok:false,reason:'boundary',delta:0,members};
      return {ok:true,reason:'ok',delta,members};
    }
