(function(root){
  ...
  function validVisualKeyframes(clip,duration){if(clip.visualKeyframes==null)return true;const frames=clip.visualKeyframes;if(!Array.isArray(frames)||frames.length>MAX_VISUAL_KEYFRAMES)return false;const times=[];for(const frame of frames){if(!frame||typeof frame!=='object'||Array.isArray(frame))return false;const time=numberValue(frame.time);if(time===null||time<0||time>duration)return false;times.push(time);for(const [key,min,max] of [['x',-200,200],['y',-200,200],['scale',.1,8],['rotation',-3600,3600],['opacity',0,1]]){if(frame[key]==null)continue;const value=numberValue(frame[key]);if(value===null||value<min||value>max)return false}if(frame.easing!=null&&(typeof frame.easing!=='string'||!KEYFRAME_EASINGS.has(frame.easing.trim().toLowerCase())))return false}times.sort((a,b)=>a-b);for(let i=1;i<times.length;i++)if(Math.abs(times[i]-times[i-1])<=.001)return false;return true}
