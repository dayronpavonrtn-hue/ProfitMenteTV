...
r.remove(c.id);assert.equal(r.list(p).length,2);r.pruneBefore('2026-08-28T00:02:00Z');assert.equal(r.list().length,3);r.clear();assert.equal(r.list().length,0);console.log('Recovery engine OK');