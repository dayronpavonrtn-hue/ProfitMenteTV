...
if(typeof document!=='undefined')(()=>{
  ...
  function retryAfterResume(reason){if(!unsaved)return false;retryCount=0;cancel();return flush(reason)}
  ...
  window.addEventListener('pageshow',()=>retryAfterResume('reanudar'));
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')lifecycleFlush('oculto');else if(document.visibilityState==='visible')retryAfterResume('visible')});
  document.addEventListener('freeze',()=>lifecycleFlush('congelado'));
  document.addEventListener('resume',()=>retryAfterResume('reanudar'));
  window.ProfitMenteProjectAutosave={engine,flush,schedule,retryAfterResume,get lastFingerprint(){return last},get unsaved(){return unsaved},get lastError(){return lastError}};
})();